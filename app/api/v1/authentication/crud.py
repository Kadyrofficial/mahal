...
from app.utils import generate_otp_code, otp_websocket
...
async def login(session: AsyncSession, login_in: LoginSchema) -> LoginSchema:
    ...
    await otp_websocket.send_otp(login_in.phone, code)
...
async def verify(session: AsyncSession, verify_in: VerifySchema) -> TokenSchema:
